...
// 定义函数指针类型
typedef bool(*Decrypt3DESFunc)(const char*, const char*, const char*);
typedef bool(*DecryptAESFunc)(const char*, const char*, const char*);
...
int main() {
    // 加载DLL
    HMODULE hDll = LoadLibraryA("../x64/Debug/Decrypt.dll");
    if (!hDll) {
